...
import JackpotModel, { EJackpotState, IJackpot } from '../models/Jackpot'
...
export default class Jackpot extends BaseService {
  ...
  constructor(io, { Provably, Affiliate, Settings, Discord }) {
    super({ location: 'service/Jackpot' })
  ...
  private async _init() {
    try {
      this.settings = await this.Settings.getJackpotConfig()
  ...
  public async placeBet(userID: Types.ObjectId, backpackIds, pendingBet = false) {
    ...
    try {
      ...
      const bet = await new Bet({
        ...
        amount,
        gameType: EGameSelection.Jackpot,
      }).save()
  ...
  private async takeRakeAndGivePrize(): Promise<void> {
    try {
      ...
      if (rakedItems.length) {
        ...
        this.Discord.Notification('ELITE Rake', '', 'Jackpot', [
          {
            name: 'GameID',
      ...
      this.Discord.Notification('FAILED RAKE OF ELITE', `Failed to rake and give prize`, 'Error', [
        {
          name: 'GameID',
          value: this.currentGame._id,
        },
        {
          name: 'Location',
          value: 'jackpot/rakeAndGivePrize()',
        },
        {
          name: 'Ping',
          value: ['admins', 'data'],
